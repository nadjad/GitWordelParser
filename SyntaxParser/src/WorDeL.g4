...
@members {
    ...
    {
    	support.initialize();
    	support.setOperators(OpReader.readOperators("src//Operators.txt"));
    }
}
...
/* ######################### TYPE DEFINITION ################################ */
tip :  'Integer' | 'String' | 'Float' | 'Boolean' ;
...
simulation returns [Simulation s]
		  : SIMULATION COLON NAME 
		    value_list NAME COLON NAME instance_ports
		    ENDFLOW
		   {$s=support.buildSimulation();};
/* */
/* flow list */	  
...
content returns [Map<String,FlowNode> flows]
		: flow_list {$flows=$flow_list.flows;} (simulation)* EOF;   // one or more flows 
