...
import representation.nodes.SwNode;
...
public class FlowDraw implements Drawable {
	...
	private void setFlow(FlowNode flow) {
		this.flow = flow;
		...
		for (int i = 0; i < size; i++) {
			...
			if (n instanceof SwNode) {
				SwNode sn = (SwNode) n;
				this.opDraws.add(new SwitchDraw(x + opOffsetX, y + opOffsetY,
						opSize, opSize, sn));
			} else {
		...
		for (Port p : outports) {
			this.outportDraws.add(new PortDraw(x + outPortOffsetX, y
					+ outPortOffsetY, portSize, portSize, p.getName(), false));
		...
		this.height += 40;
		this.representation = new Rectangle2D.Double(x, y, width, height);
		this.handler = new Rectangle2D.Double(x, y, 20, 20);
		this.resizer = new Rectangle2D.Double(x + width - 20, y + height - 20,
				20, 20);
	...
	private void createConDraws(FlowNode flow) {
