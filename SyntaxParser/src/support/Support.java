...
public class Support {
		...
		for (Port p : ports) {
			if (input) {
				...
				this.connections.put(p.getName(), new Connection(p.getName(),
						new ConnectionEnd(null, p.getName())));
			} else {
				outputPorts.put(p.getName(), p);
				this.connections.put(p.getName(), new Connection(p.getName(),
						null, new ConnectionEnd(null, p.getName())));
			}
		}
	...
	public SwNode createSwitchNode(List<String> in, String condPort,
			List<CaseItem> cases) {

		// create the node and instantiate its internal connections
		SwNode node = new SwNode(in, condPort, cases);
